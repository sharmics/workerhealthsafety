...
import "../Worker-vital-sign-details/WorkerVitalMonitor.css"
import { useLocation, useNavigate, useParams } from 'react-router-dom';

function WorkerVitalMonitor() {
    const { workerId } = useParams();
    const [fullscreenToggle, setFullscreenToggle] = useState(false)
    const navigate = useNavigate()
    const location = useLocation();
    useEffect(() => {
        if (location.pathname === `/dashboard/worker-vital-monitor/${workerId}`) {
            setFullscreenToggle(true)
        }

    }, [])
    return (
        <div className={fullscreenToggle ? 'Worker-Vital-Monitor-Outer' : "Worker-Vital-Monitor-Outer2"}>
            <div onClick={() => navigate(`/dashboard/workforce/${workerId}`)} className={fullscreenToggle ? 'Worker-Vital-BackIcon' : "Worker-Vital-BackIcon2"}>
            ...
            <div className={fullscreenToggle ? 'Worker-Vital-Monitor-ChartBox' : 'Worker-Vital-Monitor-ChartBox2'}>
                <div className={fullscreenToggle ? 'Worker-Vital-Monitor1' : "Worker-Vital-Monitor2"} >
                    <div className={fullscreenToggle ? "Worker-Vital-Monitor1-text1" : 'Worker-Vital-Monitor1-text2'}>Worker Vital Sign Monitor</div>
                    {fullscreenToggle ? "" : (<div onClick={() => navigate(`/dashboard/worker-vital-monitor/${workerId}`)} className='Worker-Vital-Monitor1-icon'><svg width="20" height="19" viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className={fullscreenToggle ? 'Worker-Vital-chart' : "Worker-Vital-chart2"}>

                    <iframe
                        className={fullscreenToggle ? 'vital-iframe' : 'vital-iframe2'}
                        title="Graph"
                        src="https://public.datapine.com/#board/BsbxSq5dL71g7SgdzIQZf5"
                        width="780px"
                        height="382px"
                        allowFullScreen
                        name='myiFrame'
...
export default WorkerVitalMonitor;
