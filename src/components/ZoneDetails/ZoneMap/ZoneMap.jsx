...
function ZoneMap() {
    ...
    useEffect(() => {
        if (location.pathname === `/dashboard/zone-map/${zoneId}`) {
            setFullscreenToggle(true)
        }

    }, [])
    return (
        <div className={fullscreenToggle ? 'ZoneMap-outer' : 'ZoneMap-outer2'}>
            {
                fullscreenToggle && <div onClick={() => navigate(`/dashboard/zone/${zoneId}`)} className='ZoneMap-BackIcon' >
            ...
            {fullscreenToggle === false &&
                <div className='ZoneMap-Large'>
                    <div className='zone-title-select'>
                        <div>Zone Map</div>
                        <select name="select" id="">
                            <option value="">Map View Toggle</option>
                        </select>
                    </div>
                    <div onClick={() => navigate(`/dashboard/zone-map/${zoneId}`)} style={{ cursor: "pointer", marginRight: "20px" }}><svg width="20" height="19" viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={fullscreenToggle ? "ZoneMap-live" : 'ZoneMap-live2'}>
                <IndoorMap />
            </div>
        </div>
    )
